...
class StructuralTimeSeriesSSM(SSM):
    ...
    def __init__(self,
        ...
        # Set the covariance matrix and the sparsing matrix for each latent component
        self.dynamics_covariance_set = {}
        self.dynamics_covariance_prior_set = {}
        self.dynamics_unc_cov_prior_set = {}
        sparsing_matrix = jnp.zeros((self.state_dim, 0))
        for i in range(self.num_blocks):
            c_prior = component_transition_covariance_priors[i]
            sparsing_block = jnp.zeros((self.state_dim, self.emission_dim))
            if c_prior is not None:
                ...
                self.dynamics_unc_cov_prior_set[f'dynamics_cov_block_{i}'] =\
                    tfd.TransformedDistribution(distribution=c_prior,
                                                bijector=PSDToRealBijector)
                sparsing_block = sparsing_block.at[
                    i*self.emission_dim: (i+1)*self.emission_dim, :].set(
                        jnp.eye(self.emission_dim))
                sparsing_matrix = jnp.concatenate((sparsing_matrix, sparsing_block), axis=1)

        self.dynamics_covariance_dense = jsp.linalg.block_diag(
            *self.dynamics_covariance_set.values())
        self.dynamics_covariance = sparsing_matrix @ self.dynamics_covariance_dense @ sparsing_matrix.T
    ...
    def emission_distribution(self, state, **covariates):
        ...
        return MVN(self.emission_matrix @ state + self.emission_input_weights @ input,
                   self.emission_covariance.value)
    ...
    def log_unc_prior(self):
        lp = jnp.array([
            self.dynamics_unc_cov_prior_set[block].log_prob(getattr(self, block).value)
            for block in self.dynamics_covariance_set.keys()
        ]).sum()
        # log prior of the emission model
        lp += self.emission_covariance_prior.log_prob(self.emission_covariance.value)
    ...
    def marginal_log_prob(self, emissions, inputs=None):
        ...
        filtered_posterior = lgssm_filter(self.lgssm_params, emissions, inputs)
    ...
    def fit_hmc(self,
        ...
        def _logprob(hmc_position, batch_emissions=batch_emissions, batch_inputs=batch_inputs):
            ...
            lp = self.log_unc_prior() + batch_lls.sum()
            return lp
        ...
        return self._to_block_covariance_matrices(param_samples)

    def _to_block_covariance_matrices(self, unconstrained_params):
        ...
        def join_params(unconstrained_parameter):
            current_params = self.lgssm_params

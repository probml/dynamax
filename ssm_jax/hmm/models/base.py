...
from ssm_jax.optimize import sample_minibatches
...
class BaseHMM(SSM):
    ...
    def marginal_log_prob(self, emissions):
        ...
        post = hmm_filter(self._compute_initial_probs(), self._compute_transition_matrices(),
    ...
    def most_likely_states(self, emissions):
        ...
        return hmm_posterior_mode(self._compute_initial_probs(), self._compute_transition_matrices(),
                                  self._compute_conditional_logliks(emissions))

    def filter(self, emissions):
        """Compute filtering distribution."""
        return hmm_filter(self._compute_initial_probs(), self._compute_transition_matrices(),
                          self._compute_conditional_logliks(emissions))

    def smoother(self, emissions):
        """Compute smoothing distribution."""
        return hmm_smoother(self._compute_initial_probs(), self._compute_transition_matrices(),
                            self._compute_conditional_logliks(emissions))
    ...
    def e_step(self, batch_emissions):
        ...
        def _single_e_step(emissions):
            ...
            posterior = hmm_two_filter_smoother(self._compute_initial_probs(), transition_matrices,
                                                self._compute_conditional_logliks(emissions))
            posterior.initial_probs = posterior.smoothed_probs[0]
    ...
    def m_step(self, batch_emissions, batch_posteriors, optimizer=optax.adam(1e-2), num_mstep_iters=50):
        ...
        params, losses = run_sgd(neg_expected_log_joint,
                                 self.unconstrained_params, (batch_emissions, batch_posteriors),
                                 optimizer=optimizer,
...
class StandardHMM(BaseHMM):
    ...
    def _m_step_emissions(self, batch_emissions, batch_posteriors, optimizer=optax.adam(1e-2), num_mstep_iters=50):
    ...
    def m_step(self, batch_emissions, batch_posteriors, optimizer=optax.adam(1e-2), num_mstep_iters=50):
        ...
        self._m_step_emissions(batch_emissions, batch_posteriors, optimizer=optimizer, num_mstep_iters=num_mstep_iters)

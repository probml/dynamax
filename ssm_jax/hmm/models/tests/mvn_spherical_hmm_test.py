...
class TestGaussianHMMWithDiagonalCovars:
    ...
    def setup(self):
        ...
        self.num_states = 4
        self.emission_dim = 3
        self.true_hmm = MultivariateNormalSphericalHMM.random_initialization(key, self.num_states, self.emission_dim)

    def test_random_initialization(self, key=jr.PRNGKey(0)):
        hmm = MultivariateNormalSphericalHMM.random_initialization(key, self.num_states, self.emission_dim)
        ...
        assert hmm._emission_cov_diag_factors.value.shape == (self.num_states, 1)

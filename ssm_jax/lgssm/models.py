...
class LinearGaussianSSM:
    ...
    p(z_t | z_{t-1}, u_t) = N(z_t | F_t z_{t-1} + B_t u_t, Q_t)
    p(y_t | z_t) = N(y_t | H_t z_t + D_t u_t, R_t)
    ...
    def sample(self, key, num_timesteps, inputs=None):
        ...
        def _step(carry, key_and_input):
            ...
            emission = MVN(H @ state + D @ u, R).sample(seed=key1)
            next_state = MVN(F @ state + B @ u, Q).sample(seed=key2)
    ...
    def log_prob(self, states, emissions, inputs=None):
        ...
        lp += (
            MVN(states[:-1] @ self.dynamics_matrix.T + inputs[:-1] @ self.dynamics_input_weights.T,
                self.dynamics_covariance)
            .log_prob(states[1:])
            .sum()
        )
        lp += (
            MVN(states @ self.emission_matrix.T + inputs @ self.emission_input_weights.T,

...
class LinearGaussianSSM:
    ...
    def __init__(
        ...
        if self.dynamics_prior is None:
            loc_d = jnp.concatenate((self.dynamics_matrix, self.dynamics_input_weights), axis=1)
            ncol_d = self.state_dim + self.input_dim
            if self._db_indicator:
                loc_d = jnp.concatenate((loc_d, self.dynamics_bias[:,None]), axis=1)
                ncol_d += 1
            self.dynamics_prior = MNIW(loc=loc_d, 
                                       col_precision=jnp.eye(ncol_d),
                                       df=self.state_dim, 
                                       scale=jnp.eye(self.state_dim))
            
        if self.emission_prior is None:
            loc_e = jnp.concatenate((self.emission_matrix, self.emission_input_weights), axis=1)
            ncol_e = self.state_dim + self.input_dim
            if self._eb_indicator:
                loc_e = jnp.concatenate((loc_e, self.emission_bias[:,None]), axis=1)
                ncol_e += 1
            self.emission_prior = MNIW(loc=loc_e, 
                                       col_precision=jnp.eye(ncol_e),
    ...
    def log_prior(self):
        d_matrix = jnp.concatenate((self.dynamics_matrix, self.dynamics_input_weights), axis=1)
        if self._db_indicator:
            d_matrix = jnp.concatenate((d_matrix, self.dynamics_bias))
        e_matrix = jnp.concatenate((self.emission_matrix, self.emission_input_weights), axis=1)
        if self._eb_indicator:
            e_matrix = jnp.concatenate((e_matrix, self.emission_bias))
        lp = self.initial_prior.logprob((self.initial_covariance, self.initial_mean))
        lp += self.dynamics_prior.logprob((self.dynamics_covariance, d_matrix))
        lp += self.emission_prior.logprob((self.emission_covariance, e_matrix))
        return lp

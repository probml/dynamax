...
class LinearGaussianSSM:
    ...
    def __init__(
        ...
        self.dynamics_bias_indicator = dynamics_bias_indicator
        self.emission_bias_indicator = emission_bias_indicator
        if self.dynamics_bias_indicator:
            self.dynamics_input_weights = jnp.hstack((self.dynamics_input_weights, jnp.ones(self.state_dim, 1)))
            dynamics_input_dim += 1
        if self.emission_bias_indicator:
    ...
    def filter(L, emissions, inputs=None):
    ...
    def _inputs_with_bias(self, num_timesteps, inputs):
        ...
        inputs_dynamics = inputs if not self.dynamics_bias_indicator else \
            jnp.hstack((inputs, jnp.ones((num_timesteps, 1))))
        
        inputs_emission = inputs if not self.emission_bias_indicator else \
            jnp.hstack((inputs, jnp.ones((num_timesteps, 1))))

...
---
colab:
  referenced_widgets: [710ee5a193064f38a00e90e10b10a624]
id: VRe-A2yV7ZCP
outputId: 542533f5-da5b-41fb-d5a0-e8d5c85c5f78
---
...
```{code-cell} ipython3
:id: 8GuHZAs17ZCQ
:outputId: 24503c33-cfd9-4bd2-cdf8-905b1e1b9048
...
fig = plt.figure(figsize=(12, 6))
ax = fig.add_subplot(1, 1, 1)
ax.plot(co2_dates, co2_by_month, lw=2, label='Observation')
ax.plot(co2_dates[-num_forecast_steps:], forecast_means, lw='2', color='orange', label='Forecast')
...
```{code-cell} ipython3
:id: 3sTB8pEU7ZCR
:outputId: 7d9ba882-6474-4daf-b94f-f42ee373a9d9

component_posterior_dict = model.decompose_by_component(co2_by_month, sts_params=parameter_samples)
...
for i, (component_name, component_posterior) in enumerate(component_posterior_dict.items()):
    ...
    ax.plot(co2_dates, component_mean, lw=2)
    ax.fill_between(co2_dates, component_mean-2.*component_std, component_mean+2.*component_std, alpha=0.5)
